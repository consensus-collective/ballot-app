...
describe('SimpleToken', function () {
  ...
  beforeEach(async function () {
    ...
    const ballotFactory = await ethers.getContractFactory('Ballot2')
    ballotContract = await ballotFactory.connect(owner).deploy(proposals)
  ...
  describe('[event]', function () {
    ...
    it.only('shd emit Delegate', async function () {
      // give voting right to user0
      await ballotContract.giveRightToVote(users[0])
